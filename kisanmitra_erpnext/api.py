...
def update_communication():
	...
	try:
		...
		calls = frappe.get_all("Communication", fields=["*"], filters={"comment_type":"Info"}, order_by='creation desc')
		for call in calls:
	...
	try:
		if doc.get("contact"):
			callers_name = frappe.db.sql("""select trim(concat(first_name," ",last_name)) as caller_name
				from `tabContact` where name=%s""",(doc.get("contact")),as_dict=1)[0]
			frappe.db.set_value(doc.doctype,doc.name,"km_caller_name",callers_name.get("caller_name"))
		
		elif doc.get("raised_by_phone"):
			callers_name = frappe.db.sql("""select trim(concat(first_name," ",last_name)) as caller_name
				from `tabContact` where mobile_no=%s""",(doc.get("raised_by_phone")),as_dict=1)[0]
			frappe.db.set_value(doc.doctype,doc.name,"km_caller_name",callers_name.get("caller_name"))

		elif doc.get("lead"):
			callers_name = frappe.db.sql("""select trim(lead_name) as caller_name
				from `tabLead` where name=%s""",(doc.get("lead")),as_dict=1)[0]
			frappe.db.set_value(doc.doctype,doc.name,"km_caller_name",callers_name.get("caller_name"))
	except Exception as e:
		frappe.db.rollback()
		frappe.log_error(message=frappe.get_traceback(), title="Error in updating communication")
