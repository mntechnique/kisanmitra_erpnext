	...
	try:
		...
		for call in calls:
			if not call.get("km_calls_end_date"):
				response = requests.get('https://{exotel_sid}:{exotel_token}@api.exotel.com/v1/Accounts/{exotel_sid}/Calls/{call_sid}.json'.format(exotel_sid = credentials.exotel_sid,exotel_token = credentials.exotel_token,call_sid = call.get("CallSid")))
